...
export function CartDrawer({ mesaId, mesaNumero, pedidoAtivo, onClose }: CartDrawerProps) {
  ...
  // Assinar 'carrinhos' como objeto (referência muda a cada mutação) para re-render
  // e ler itens via getter imperativo — evita novo [] a cada render
  const carrinhos = useCarrinhoGarcomStore((state) => state.carrinhos)
  const rascunho = isMounted ? (carrinhos[mesaId]?.itens ?? []) : []
